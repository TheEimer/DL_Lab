...
with tf.device("/device:GPU:0"):
    ...
    def cnn_model_fn(features, labels, num_filters, learning_rate, mode):
    ...
    max_epochs = 1
    axis = np.arrange(max_epochs)
    plt.figure()
    color = cm.rainbow(np.linspace(0, 1, len(rates))
    for rate, c in zip(rates, color):
        ...
        for i in range(max_epochs):
            ...
            eval_input_fn = tf.estimator.inputs.numpy_input_fn(
                ...
                num_epochs=1,
                shuffle=False)
            losses[i] = mnist_cnn.evaluate(input_fn=eval_input_fn)
